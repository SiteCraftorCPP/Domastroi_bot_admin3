...
async def show_page(message: types.Message, page: int, edit: bool = False):
    if page == 1:
        ...
        text = "Текст для второй страницы с изображением."
        ...
        text = "Текст для третьей страницы с видео."
...
async def show_user_bot_info(callback_query: types.CallbackQuery):
    ...
    await bot.answer_callback_query(callback_query.id)
    await callback_query.message.answer(f"Информация о боте пользователя с ID {user_id}.")
